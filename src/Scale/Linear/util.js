...
define(function (require) {
    ...
    function makeRange(start, end, step) {
        ...
        if (ascending) {
            while (sum <= end) {
                arr.push(sum);
                sum = mathUtil.accAdd(sum, step);
            }
        } else {
            while (sum >= end) {
                arr.push(sum);
                sum = mathUtil.accSub(sum, step);
            }
        }
        return arr;
    }
    ...
    function tickIncrement(minValue, maxValue, count) {
        ...
        // 这里的计算，如果是小数的话有可能会使得newStep的后面加上一个很小很小的数，
        // 这里解决一下
        newStep = mathUtil.accMul(newStep, Math.pow(10, magnitude));
     ...
     * @returns {Function}
     */
    function deInterpolateNumber(a, b, clamp) {
        clamp = !!clamp;
        return function (x) {
            var t = (x - a) / (b - a);
    ...
    return {
        ...
        deInterpolateNumber: deInterpolateNumber,
        reInterpolateNumber: reInterpolateNumber
    };
